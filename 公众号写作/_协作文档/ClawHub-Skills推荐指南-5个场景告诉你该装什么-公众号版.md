...
| 你是谁 | 推荐 Skills | 核心价值 |
|--------|------------|----------|
| 自媒体 / 内容创作者 | Telegram Bot + Umami Stats | 内容分发 + 数据监控 |
| 个人财务管理 | BillClaw + KameleonDB | 自动记账 + 数据存储 |
| 量化 / 交易玩家 | Turbine Bot + Cron Dashboard | 预测市场 + 定时执行 |
| 开发者 / 安全部落 | Skill Auditor Pro + Scan Skill | 安全审查 + 恶意检测 |
| 日常效率党 | Microsoft To Do + Quick Reminders | 任务管理 + 快速提醒 |
